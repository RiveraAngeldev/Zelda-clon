...
void Map::checkCollision(){
    ...
    // Item collision sword
    for(auto it: currentSection->getItems()){
        for(auto item: it) {
            if (item == NULL) continue;
            if (item->collidesWith(link)){
                item->onCollision(link);
            }
        }
    }
    //Make the sword item disappear

    for(auto it: currentSection->getItems()){
        for(auto item: it) {
            if (item == NULL) continue;
            if (item->getIsCollected() && getPlayer()->getIsPickingUp() == false) {
                item->setY(8000);
            }
        }
    } 

    //Makes it so that the enemies are hit by the sword
    for(auto it: currentSection->getEnemies()){
        if(it == NULL)continue;
        if(getPlayer()->getSwordHitbox().intersects(it->getBounds())){
            it->damage(1);
        }
    }

    //Makes it so that Link gets damaged when hit
    for(auto it: currentSection->getEnemies()){
        if(it == NULL)continue;
        if(!(it->isDead())){
            if(getPlayer()->getBounds().intersects(it->getBounds())){
                //if statements that determine enemies damage
                if(it->type=="onehit"){
                    getPlayer()->damage(9000); //this one is for the one shot enemy
                }
                else{
                    getPlayer()->damage(1);
                }
