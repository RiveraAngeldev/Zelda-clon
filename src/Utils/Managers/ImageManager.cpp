...
ImageManager::ImageManager() {
    ...
    dungeonPixelMap = ofImage("images/Maps/newDungeon.png");
...
shared_ptr<DynamicEntityAnimations> ImageManager::getOneHitAnimations(){
    DirectionalAnimations movement;
    
    // OneHit's movement animation is the same for all directions
    movement.down = getAnimationFromIndices(105,106);
    movement.right = movement.down;
    movement.left = movement.down;
    movement.up = movement.down;

    OneHitAnimations = make_shared<DynamicEntityAnimations>();

    OneHitAnimations->movement = movement;

    // No attacking animation for OneHit 
    // At least I didnt find one.
    OneHitAnimations->attacking = movement;

    OneHitAnimations->damage = getAnimationFromIndices(88, 89);

    return OneHitAnimations;
