...
void GameState::update()
{
    ...
    this->onehit->linkX = currentMap->getPlayer()->getX();
    this->onehit->linkY = currentMap->getPlayer()->getY();
...
void GameState::reset() {
   ...
   //Sword Spawn coords
   oldManCaveMap->getCurrentSection()->addItemToTile(8, 5, static_pointer_cast<Item>(make_shared<BasicSword>(4, 4, handler, handler->getImageManager()->basicSword)));
    ...
    overWorldMap->getSection(3, 0)->addEnemyToTile(4,8, onehit);
...
//For making the one shot enemy
shared_ptr<OneHit> GameState::createOneHit(){
    return make_shared<OneHit>(0, 0, handler, handler->getImageManager()->getOneHitAnimations());
}
